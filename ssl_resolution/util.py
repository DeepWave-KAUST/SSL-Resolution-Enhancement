...
def lowpass_filter(data, freq, dt, pad=None):
    """Perform low-pass filter to a data
    Args:
    ...
    Returns:
        array_like: Low-pass filtered data.
    """

    if pad is not None:
        data = F.pad(data, (pad, pad, pad, pad), mode='replicate')
    sos = signal.butter(8, freq / (0.5 * (1 / dt)), btype='lowpass', output='sos')
